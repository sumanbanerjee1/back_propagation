...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 10 01:08:19 2017
...
def data_stats(data): # can be coded up at a later stage
	...
	return [max(pre_len),max(kb_len),max(post_len),
         max(utterances_len),max(utterances_len),max(utterances_len)]
...
def prepro(args):
	...
	total_stats=[max(test_stats[0],max(train_stats[0],dev_stats[0])),max(test_stats[1],max(train_stats[1],dev_stats[1])),
                 ...
                 max(test_stats[4],max(train_stats[4],dev_stats[4])),max(test_stats[5],max(train_stats[5],dev_stats[5]))]
